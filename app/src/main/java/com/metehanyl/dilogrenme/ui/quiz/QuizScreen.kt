...
fun QuizScreen(
    ...
    val engine = remember(nativeLang, targetLang) {
        QuizEngine(wordBank.words, wordBank.languages, nativeLang, targetLang)
    }
    var question by remember(engine) {
        mutableStateOf(engine.nextQuestion(randomDirection()))
    }
    ...
    QuizQuestionView(
        ...
        onSpeak = { speakPrompt() },
        onNext = {
            question = engine.nextQuestion(randomDirection())
            selectedIndex = null
        },
        onBack = onBack
    )
...
private fun randomDirection(): QuizMode =
    if (Random.nextBoolean()) QuizMode.WORD_TO_MEANING else QuizMode.MEANING_TO_WORD
